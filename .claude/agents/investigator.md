...
- Phase 3: `boot` → `1-phase12-read` → `2-ui-semantic-map` → `3-locator-catalog` → `4-assessment-write` → `done`
...
3. interactive 要素を抽出 → `interactive_element_catalog.json`
4. locator 候補を AC 別に整理 → `locator_catalog.json` (evaluator per-ac Step 0 が literal 引用する)
5. screen 構造の outline → `screen_structure_outline.md` (人間可読サマリ)
6. 画面間遷移 (state transition) → `state_transition_hint.json`
