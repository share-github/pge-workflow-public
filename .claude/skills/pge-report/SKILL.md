---
description: PGE 成果物（spec.md・feedback/final.json・<SUT root>/evidence/<test>/）から AC × Evidence の単一 HTML レポートを生成し、ブラウザでサムネ一覧＋クリック拡大表示で人間レビューしやすい形式に変換する。PGE flow から責務分離された手動実行ツール。
...
PGE が出力した成果物（`plan/spec.md`・`plan/feedback/<name>.json`・`<SUT root>/evidence/<test>/`・Phase X3 + D-4 以降）を **単一 HTML レポート** に変換するスキル。読み込み後にやることは 1 つだけ：`.claude/tools/pge-report.py` を実行してレポートを生成する。
...
1. **入力ファイルの存在確認** — `plan/spec.md` / `plan/feedback/<feedback>.json` がワークスペースに揃っているか確認する (D-4: `.md` ファイルは不要・aggregator が生成しない)。`<SUT root>/evidence/` の attachments は `feedback.json` の `ac_coverage[].attachments_dir` から SUT root 相対パスで参照される (Phase X3 規約)。揃っていなければユーザーに通知し中断（PGE が未完了の可能性）。
...
| `--plan-dir <path>` | `<workspace>/plan` | spec.md / feedback/ の親ディレクトリ |
...
|---|---|---|
| `missing input: ...plan/spec.md` | PGE 未完了 or workspace パスが違う | `--workspace` を見直す、または PGE を完了させてから再実行 |
| `missing input: ...plan/feedback/final.json` | Evaluator final 未実行 or 別の feedback name | `--feedback sprint-N` を試すか PGE を進める |
| `no ACs parsed from spec.md` | spec.md の AC 行が `- AC-N [TAG]: When ... the ...` 形式に従っていない | Planner 出力フォーマットを確認 |
...
- spec.md の AC 形式が不明（Planner 出力フォーマットの変更 → `.claude/skills/pge-planning/SKILL.md` 「仕様書の出力フォーマット」を見直すか）
