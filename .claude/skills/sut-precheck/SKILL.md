...
| 8 (Phase Z3+) | SUT root の `playwright.config.ts` (or `.js` / `.cjs`) の `use:` block に **`trace: 'on-first-retry'`** が含まれる (Step 10 retry で trace bundle を LLM input 化するための必須設定) | PJ owner が `playwright.config.ts` の `use:` block に `trace: 'on-first-retry', screenshot: 'only-on-failure'` を追加 (詳細は [`.claude/references/playwright-fixture-template.md`](../../references/playwright-fixture-template.md) `4. PJ-level config 規約` 節) |
