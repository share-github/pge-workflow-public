...
```json
{
  ...
  "test_runner": {
    ...
    "browsers": ["<browser>", ...],
    "fixtures_module_path": "<SUT root relative path to PGE fixtures module・default '.pge-fixtures'・Playwright artifact が import 先 path を machine 算出するために使う>",
    "noise_filter": {
      "network_abort": ["<glob or regex pattern>", "..."],
      "console_suppress_pattern": "<regex pattern as single string・空文字なら何も suppress しない>"
    }
  },
  "regressive_fix_scan": {
    "groups": [
      {
        "id": "<short identifier for the group (e.g. 'client-view' / 'server-validation' / 'auth-decorator')・bash safe (lowercase + hyphen)>",
        "description": "<このグループが保護する規約の 1 行説明>",
        "file_globs": ["<git diff -- に渡す glob pattern>", "..."],
        "removed_token_regex": "<grep -E に渡す regex pattern (1 本)・このパターンの文字列が削除行に含まれていれば退化的修正とみなす>"
      }
    ]
  },
  "parallel_db": {
...
#### `test_runner.fixtures_module_path`

Playwright fixture module (`.pge-fixtures` 系) の SUT root relative path。test artifact (`e2e/sprint-N/AC-K.spec.ts`) から本 module を import する際の relative path 計算に使う。default `.pge-fixtures` (SUT root 直下に配置・拡張子は test runner が補う)。

evaluator-per-ac は test artifact 出力 dir (`e2e/sprint-N/`) と本 field の組合せから `path.relative` で import path を deterministic 算出する (LLM 推論禁止)。SUT root 以外への配置が必要な PJ (monorepo / 別 module 配下 等) は本 field で path を上書きする。

#### `regressive_fix_scan`

Phase Z9+ 新規 (旧 generator.md 4.6-A の hardcoded literal patterns を本 field へ migrate)。退化的修正 (HTML 属性削除 / validation annotation 削除 / security 制約 bypass 等) を git diff で検出するための **PJ 別 pattern catalog**。

groups[] の各 entry は **独立した scan group** で、それぞれ `file_globs[]` (git diff フィルタ) と `removed_token_regex` (削除行 grep) を持つ。Generator は全 group を順に grep し、いずれか 1 group で match があれば retry 時の commit を halt する。

`groups[]` が空配列 or 本 field 未定義 → Generator 4.6-A は warning log で skip (= 退化的修正 detect は dis-armed)。PJ owner が pattern を declare していない PJ では本 self-check は無効化される (= 退化的修正検知の責任を `/pge-convention-survey` の bundle・PJ CI 側の lint に委譲)。

PJ owner は `/pge-runtime-survey` の対話で本 group 群を declare する。stack-specific な pattern (Java/Kt の Jakarta Bean Validation annotation、Python decorator、Ruby validates DSL 等) は本 field に PJ 値として埋め込む (= framework 側にこの種の literal を hardcode しない)。

#### `test_runner.noise_filter`

Phase Z3+ で導入。E2E test 実行時に **`stderr_excerpt` / console log に混入する dev 環境ノイズ** (favicon 404 / HMR warning / vite warning / analytics 失敗 等) を Playwright fixture 層で一括 mute する宣言。

- `network_abort[]`: Playwright `page.route(pattern, r => r.abort())` で network request 自体を block する pattern 配列。各 entry は Playwright が受け付ける URL pattern (glob `**/...` or RegExp 文字列)。Default 空配列 (何も block しない)。
- `console_suppress_pattern`: Playwright `page.on('console', ...)` で **filter する** console message の content regex (単一 pattern を文字列で declare)。Default 空文字 (何も suppress しない)。

宣言された pattern は orchestrator が `.pge-fixtures.ts` (SUT root 配下に生成される PGE fixture fragment) に literal 注入する。PJ の `playwright.config.ts` 自体は本 fixture を import する規約に従う (詳細は [`.claude/references/playwright-fixture-template.md`](./playwright-fixture-template.md))。

**設計指針**: PJ 固有ではない universal な noise pattern (favicon / browser default fetch / HMR 等) のみ declare すること。PJ 固有 endpoint / business route を block すると test の本質が壊れる。
...
| `test_runner.fixtures_module_path` | `".pge-fixtures"` (SUT root 直下) |
| `test_runner.noise_filter.network_abort` | `[]` (空配列・何も block しない) |
| `test_runner.noise_filter.console_suppress_pattern` | `""` (空文字・何も suppress しない) |
| `regressive_fix_scan.groups` | `[]` (空配列・退化的修正 detect は dis-armed) |
